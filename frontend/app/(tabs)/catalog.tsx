...
// Skeleton placeholder component for instant render
const SkeletonCard = () => (
...
export default function CatalogScreen() {
  ...
  // CRITICAL: Only show skeleton on FIRST load, not on refocus
  ...
  // OPTIMIZED: Categories computed from offers (memoized)
  ...
  // Load data - silently refresh when screen focuses (no blocking spinner)
  useFocusEffect(
    useCallback(() => {
      if (isFirstLoad.current) {
        // First load - show skeleton briefly
        loadData(true);
        isFirstLoad.current = false;
      } else {
        // Subsequent visits - silent background refresh
        loadData(false);
      }
    }, [])
  );
  ...
  const loadData = async (showLoading: boolean) => {
    ...
    try {
      ...
      const newZones = catalogData.zones || [];
      const newOffers = catalogData.offers || [];
      ...
      // Set zone only if needed
      if (newZones.length > 0 && !selectedZone) {
  ...
  // Handle zone change
  useEffect(() => {
    if (selectedZone && !initialLoading) {
  ...
  // MEMOIZED: Active bids
  const activeBids = useMemo(() => {
    return filteredOffers.filter(o => o.current_aggregated_qty > 0);
  }, [filteredOffers]);

  // MEMOIZED: Products without offers
  ...
  const getCurrentPrice = (slabs: QuantitySlab[], qty: number) => {
    ...
    return slabs[0]?.price_per_unit || 0;
  };
  ...
  const getNextSlabInfo = (slabs: QuantitySlab[], currentQty: number) => {
    ...
    const currentPrice = slabs[currentSlabIndex]?.price_per_unit || 0;
      ...
      >
        ...
        <View style={styles.priceSection}>
          <View style={styles.currentPriceBox}>
            ...
            <Text style={styles.currentPriceValue}>₹{nextSlabInfo.currentPrice}/{offer.product_unit}</Text>
            <Text style={styles.aggregatedQty}>{offer.current_aggregated_qty} {offer.product_unit} ordered</Text>
          </View>
          ...
          {nextSlabInfo.hasNextSlab ? (
            <View style={styles.nextSlabBox}>
              ...
              <Text style={styles.nextSlabPrice}>₹{nextSlabInfo.nextPrice}/{offer.product_unit}</Text>
              <View style={styles.savingsTag}>
                <Text style={styles.savingsText}>Add {nextSlabInfo.qtyNeeded} more to save ₹{nextSlabInfo.savings}/unit</Text>
              </View>
            </View>
          ) : (
        ...
        <View style={styles.progressSection}>
          <View style={styles.progressHeader}>
            <Text style={styles.progressLabel}>{offer.current_aggregated_qty}/{offer.min_fulfillment_qty} for fulfillment</Text>
  ...
  const renderProductCard = (product: Product) => (
    <View key={product.id} style={styles.productCard}>
      ...
      <TouchableOpacity style={styles.requestBidButton} onPress={() => openBidModal(product)}>
  ...
  // RENDER: Main content - always show UI, use skeleton only on first load
  return (
    <SafeAreaView style={styles.container} edges={['bottom']}>
      {/* Zone Selector - ALWAYS visible */}
      <TouchableOpacity style={styles.zoneSelector} onPress={() => setZoneModalVisible(true)}>
        <Ionicons name="location" size={20} color="#6c5ce7" />
        <Text style={styles.zoneSelectorText}>{selectedZone?.name || 'Select Zone'}</Text>
      ...
      <View style={styles.tabContainer}>
        ...
        >
          <Ionicons name="flame" size={18} color={activeTab === 'active' ? '#fff' : '#666'} />
          <Text style={[styles.tabText, activeTab === 'active' && styles.tabTextActive]}>
            Active Bids ({initialLoading ? '...' : activeBids.length})
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          <Ionicons name="grid" size={18} color={activeTab === 'browse' ? '#fff' : '#666'} />
      ...
      {activeTab === 'browse' && categories.length > 0 && (
        ...
        >
          ...
          >
            <Text style={[styles.categoryText, !filterCategory && styles.categoryTextActive]}>All</Text>
          </TouchableOpacity>
          {categories.map((cat) => (
            <TouchableOpacity
              key={cat}
      ...
      {/* Content - Show skeleton ONLY on first load */}
      <ScrollView
        style={styles.content}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#6c5ce7" />}
      >
        {initialLoading ? (
          // Skeleton loading - only on first load
          <>
            <SkeletonCard />
            <SkeletonCard />
            <SkeletonCard />
          ...
          activeBids.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtitle}>Browse products and join group orders to see active bids here</Text>
          ...
          <>
            {filteredOffers.length > 0 && (
              <>
                ...
                {filteredOffers.map(offer => renderOfferCard(offer, true))}
              </>
            )}
            {productsWithoutOffers.length > 0 && (
              <>
                <Text style={styles.sectionTitle}>Request a Bid</Text>
                <Text style={styles.sectionSubtitle}>These products don't have active offers. Request a bid!</Text>
                {productsWithoutOffers.map(renderProductCard)}
              </>
            )}
            {filteredOffers.length === 0 && productsWithoutOffers.length === 0 && (
              <View style={styles.emptyState}>
          ...
          <View style={[styles.modalContent, { paddingBottom: insets.bottom + 24 }]}>
            ...
            {zones.map((zone) => (
              <TouchableOpacity
                ...
                onPress={() => { setSelectedZone(zone); setZoneModalVisible(false); }}
              >
                <Ionicons
          ...
          <View style={[styles.modalContent, { paddingBottom: insets.bottom + 24 }]}>
            {selectedOffer && (
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <Text style={styles.slabSubtitle}>Currently at {selectedOffer.current_aggregated_qty} {selectedOffer.product_unit} (zone total)</Text>
                {selectedOffer.quantity_slabs.map((slab, idx) => {
                  ...
                  return (
                    <View key={idx} style={[styles.slabRow, isCurrentSlab && styles.slabRowCurrent]}>
                      <View style={styles.slabRowLeft}>
                        {isCurrentSlab && <Ionicons name="arrow-forward" size={14} color="#6c5ce7" style={styles.slabArrow} />}
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.bidForm}>
                  ...
                  <TextInput
                    ...
                    placeholder="e.g., Need by next week..."
                    placeholderTextColor="#666"
                    multiline
                    numberOfLines={3}
                  />
                ...
                <View style={styles.bidInfo}>
                  ...
                  <Text style={styles.bidInfoText}>
                    Your request will be reviewed. We'll notify you when a supplier offer is available.
                  </Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  skeletonIcon: { width: 48, height: 48, borderRadius: 12, backgroundColor: '#2d2d44' },
  skeletonTextGroup: { flex: 1, marginLeft: 12 },
  skeletonTitle: { height: 16, width: '60%', backgroundColor: '#2d2d44', borderRadius: 4, marginBottom: 8 },
  skeletonSubtitle: { height: 12, width: '40%', backgroundColor: '#2d2d44', borderRadius: 4 },
  skeletonPrice: { height: 40, backgroundColor: '#2d2d44', borderRadius: 8, marginBottom: 12 },
  skeletonProgress: { height: 6, backgroundColor: '#2d2d44', borderRadius: 3 },
  ...
  zoneSelectorText: { flex: 1, color: '#fff', fontSize: 16 },
  ...
  categoryText: { color: '#a0a0a0', fontSize: 14 },
  categoryTextActive: { color: '#fff' },
  
  // Content
  content: { flex: 1, padding: 16 },
  sectionTitle: { color: '#fff', fontSize: 18, fontWeight: 'bold', marginTop: 8, marginBottom: 12 },
  sectionSubtitle: { color: '#a0a0a0', fontSize: 14, marginBottom: 16, marginTop: -8 },
  
  // Empty state
  emptyState: { alignItems: 'center', justifyContent: 'center', paddingVertical: 64 },
  emptyTitle: { color: '#fff', fontSize: 20, fontWeight: 'bold', marginTop: 16 },
  emptySubtitle: { color: '#a0a0a0', fontSize: 14, textAlign: 'center', marginTop: 8, paddingHorizontal: 32 },
  browseButton: { backgroundColor: '#6c5ce7', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 12, marginTop: 20 },
  browseButtonText: { color: '#fff', fontSize: 16, fontWeight: '600' },
  
  // Offer card
  offerCard: { backgroundColor: '#1a1a2e', borderRadius: 16, padding: 16, marginBottom: 12 },
  offerHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' },
  productInfo: { flexDirection: 'row', flex: 1 },
  productImage: { width: 48, height: 48, borderRadius: 12, backgroundColor: '#0f0f1a' },
  productIcon: {
  ...
  productBrand: { color: '#a0a0a0', fontSize: 14, marginTop: 2 },
  supplierBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
  supplierCode: { color: '#fff', fontSize: 12, fontWeight: 'bold' },
  ...
  currentPriceBox: { flex: 1, backgroundColor: '#0f0f1a', borderRadius: 12, padding: 12, alignItems: 'center' },
  currentPriceLabel: { color: '#a0a0a0', fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.5 },
  currentPriceValue: { color: '#6c5ce7', fontSize: 20, fontWeight: 'bold', marginTop: 4 },
  aggregatedQty: { color: '#666', fontSize: 11, marginTop: 4 },
  nextSlabBox: {
    flex: 1,
  ...
  nextSlabHeader: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  nextSlabTitle: { color: '#27ae60', fontSize: 11, textTransform: 'uppercase', letterSpacing: 0.5 },
  nextSlabPrice: { color: '#27ae60', fontSize: 20, fontWeight: 'bold', marginTop: 4 },
  savingsTag: { marginTop: 6 },
  savingsText: { color: '#27ae60', fontSize: 10, fontWeight: '600' },
  bestPriceBox: {
    flex: 1,
    backgroundColor: 'rgba(39, 174, 96, 0.1)',
    borderRadius: 12,
  ...
  bestPriceText: { color: '#27ae60', fontSize: 14, fontWeight: 'bold' },
  
  // Progress
  progressSection: { marginTop: 12 },
  progressHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 },
  progressLabel: { color: '#a0a0a0', fontSize: 12 },
  progressPercent: { color: '#6c5ce7', fontSize: 12, fontWeight: '600' },
  progressBarBg: { height: 6, backgroundColor: '#2d2d44', borderRadius: 3, overflow: 'hidden' },
  progressBarFill: { height: '100%', backgroundColor: '#6c5ce7', borderRadius: 3 },
  progressBarComplete: { backgroundColor: '#27ae60' },
  readyBadge: { flexDirection: 'row', alignItems: 'center', marginTop: 8, gap: 4 },
  readyText: { color: '#27ae60', fontSize: 12, fontWeight: '600' },
  activeBidBadge: { flexDirection: 'row', alignItems: 'center', marginTop: 8, gap: 4 },
  activeBidText: { color: '#f39c12', fontSize: 12, fontWeight: '600' },
  leadTimeRow: { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 12 },
  leadTimeText: { color: '#666', fontSize: 12 },
  
  // Product card
  productCard: { backgroundColor: '#1a1a2e', borderRadius: 16, padding: 16, marginBottom: 12 },
  productCardHeader: { flexDirection: 'row', alignItems: 'center' },
  productCardInfo: { flex: 1, marginLeft: 12 },
  productUnit: { color: '#666', fontSize: 12, marginTop: 2 },
  requestBidButton: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
  ...
  requestBidText: { color: '#fff', fontSize: 14, fontWeight: '600' },
  
  // Modal
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.7)', justifyContent: 'flex-end' },
  modalContent: { backgroundColor: '#1a1a2e', borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, maxHeight: '90%' },
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 },
  modalTitle: { color: '#fff', fontSize: 20, fontWeight: 'bold', flex: 1, marginRight: 16 },
  imageScroll: { marginBottom: 16 },
  modalImage: { width: 120, height: 120, borderRadius: 12, marginRight: 12, backgroundColor: '#0f0f1a' },
  ...
  zoneOptionSelected: { borderWidth: 1, borderColor: '#6c5ce7' },
  zoneOptionText: { color: '#fff', fontSize: 16 },
  
  // Offer detail
  offerDetailSection: { backgroundColor: '#0f0f1a', borderRadius: 12, padding: 16, marginBottom: 16 },
  detailRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#2d2d44' },
  detailLabel: { color: '#a0a0a0', fontSize: 14 },
  detailValue: { color: '#fff', fontSize: 14, fontWeight: '500' },
  
  // Slabs
  slabTitle: { color: '#fff', fontSize: 16, fontWeight: '600', marginBottom: 4 },
  slabSubtitle: { color: '#a0a0a0', fontSize: 12, marginBottom: 12 },
  slabRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#0f0f1a', padding: 12, borderRadius: 8, marginBottom: 8 },
  slabRowCurrent: { backgroundColor: 'rgba(108, 92, 231, 0.15)', borderWidth: 1, borderColor: '#6c5ce7' },
  slabRowLeft: { flexDirection: 'row', alignItems: 'center' },
  slabArrow: { marginRight: 8 },
  slabRowRight: { alignItems: 'flex-end' },
  slabQty: { color: '#a0a0a0', fontSize: 14 },
  slabQtyCurrent: { color: '#fff', fontWeight: '600' },
  slabPrice: { color: '#27ae60', fontSize: 14, fontWeight: '600' },
  slabPriceCurrent: { color: '#6c5ce7', fontSize: 16, fontWeight: 'bold' },
  currentSlabTag: { backgroundColor: '#6c5ce7', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, marginTop: 4 },
  currentSlabTagText: { color: '#fff', fontSize: 9, fontWeight: 'bold' },
  ...
  fulfillmentText: { color: '#a0a0a0', fontSize: 12, flex: 1 },
  
  // Quantity section
  quantitySection: { marginTop: 20, backgroundColor: '#0f0f1a', borderRadius: 12, padding: 16 },
  quantityLabel: { color: '#fff', fontSize: 16, fontWeight: '600', marginBottom: 12 },
  quantityInputRow: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  quantityButton: { width: 48, height: 48, backgroundColor: '#6c5ce7', borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  quantityInput: {
    flex: 1,
    backgroundColor: '#1a1a2e',
    borderRadius: 12,
  ...
  pricePreview: { marginTop: 16, alignItems: 'center' },
  pricePreviewLabel: { color: '#a0a0a0', fontSize: 12 },
  pricePreviewValue: { color: '#27ae60', fontSize: 24, fontWeight: 'bold' },
  totalPreview: { color: '#6c5ce7', fontSize: 16, fontWeight: '600', marginTop: 4 },
  ...
  orderButtonDisabled: { backgroundColor: '#4a4a5e' },
  orderButtonText: { color: '#fff', fontSize: 16, fontWeight: '600' },
  
  // Bid modal
  bidProductInfo: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#0f0f1a', padding: 16, borderRadius: 12, marginBottom: 20 },
  bidProductImage: { width: 64, height: 64, borderRadius: 12, backgroundColor: '#1a1a2e' },
  bidProductIcon: { width: 64, height: 64, borderRadius: 12, backgroundColor: 'rgba(108, 92, 231, 0.2)', alignItems: 'center', justifyContent: 'center' },
  bidProductDetails: { flex: 1, marginLeft: 12 },
  bidProductName: { color: '#fff', fontSize: 18, fontWeight: '600' },
  bidProductBrand: { color: '#a0a0a0', fontSize: 14, marginTop: 2 },
  bidProductCategory: { color: '#666', fontSize: 12, marginTop: 2 },
  bidForm: { marginBottom: 16 },
  inputLabel: { color: '#fff', fontSize: 14, marginBottom: 8 },
  bidInput: { backgroundColor: '#0f0f1a', borderRadius: 12, padding: 16, color: '#fff', fontSize: 16, borderWidth: 1, borderColor: '#2d2d44', marginBottom: 16 },
  bidNotesInput: { height: 100, textAlignVertical: 'top' },
  bidInfo: { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: 'rgba(243, 156, 18, 0.1)', padding: 12, borderRadius: 12, gap: 8, marginBottom: 8 },
  bidInfoText: { color: '#a0a0a0', fontSize: 12, flex: 1 },
  submitBidButton: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
